                        ...
                        <tr>
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                <p class="text-gray-900 whitespace-no-wrap">
                                    <?php 
                                        // Kiểm tra nếu có cột ngay_tao hoặc ngay_mua
                                        echo isset($row['ngay_tao']) ? $row['ngay_tao'] : (isset($row['ngay_mua']) ? $row['ngay_mua'] : 'N/A'); 
                                    ?>
                                </p>
                            </td>
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                <span class="relative inline-block px-3 py-1 font-semibold leading-tight">
                                    <?php 
                                        ...
                                        // Giả sử trạng thái lưu số: 0: Chờ, 1: Đã duyệt, 2: Đang giao, 3: Hoàn thành
                                        if (isset($row['trang_thai'])) {
                                            if ($row['trang_thai'] == 1) { $statusClass = 'text-blue-900 bg-blue-200'; $statusText = 'Đã duyệt'; }
                                            elseif ($row['trang_thai'] == 2) { $statusClass = 'text-orange-900 bg-orange-200'; $statusText = 'Đang giao'; }
                                            elseif ($row['trang_thai'] == 3) { $statusClass = 'text-green-900 bg-green-200'; $statusText = 'Hoàn thành'; }
                                        }
                                    ?>
                                    <span aria-hidden class="absolute inset-0 <?php echo $statusClass; ?> opacity-50 rounded-full"></span>
                                    <span class="relative <?php echo $statusClass; ?>"><?php echo $statusText; ?></span>
                                </span>
                            </td>
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
